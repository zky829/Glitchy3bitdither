(function() {
  var andSort, mask, masks, numericSort, orSort, randminmax, randomSort, slice_range, sum, swapEnd32, xorSort;
  ...
  Caman.Filter.register('shortNumericSort', function() {
    ...
    da.sort(numericSort);
    return this.pixelData.data.set(da, mm[0]);
  });
  ...
  Caman.Filter.register('sort', function() {
    var da, data, mm;
    data = new Uint32Array(this.pixelData);
  ...
  Caman.Filter.register('bettersort', function() {
    var cut, data, mm;
    data = new Uint32Array(this.pixelData);
    mm = randminmax(0, data.length);
    cut = data.subarray(mm[0], mm[1]);
    Array.prototype.sort.call(cut, numericSort);
    return this.pixelData.data.set(data.buffer);
  });
