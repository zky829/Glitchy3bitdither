(function() {
  ...
  Caman.Filter.register('shortNumericSort', function() {
    return this.processPlugin('shortNumericSort', []);
  });

  Caman.Plugin.register('shortNumericSort', function() {
    var da, data, mm;
    ...
    da.sort(Util.numericSort);
    this.pixelData.data.set(da, mm[0]);
    return this;
  });

  Caman.Filter.register('shortDumbSort', function() {
    return this.processPlugin('shortDumbSort', []);
  });

  Caman.Plugin.register('shortDumbSort', function() {
    var da, data, mm;
    data = new Uint32Array(this.pixelData);
    mm = Util.slice_range(this.dimensions.width, this.dimensions.height, 1);
    da = Array.apply([], data.subarray(mm[0], mm[1]));
  ...
  Caman.Filter.register('sortA', function() {
    return this.processPlugin('sortA', []);
  });

  Caman.Plugin.register('sortA', function() {
    var da, data, mm;
    data = new Uint32Array(this.pixelData);
  ...
  Caman.Filter.register('sortB', function() {
    return this.processPlugin('sortB', []);
  });

  Caman.Plugin.register('sortB', function() {
    var cut, data, mm;
    data = new Uint32Array(this.pixelData);
    mm = Util.randminmax(0, data.length);
  ...
  Caman.Plugin.register('shortsort', function() {
    var cut, data, mm;
    data = new Uint32Array(this.pixelData.buffer);
    mm = Util.slice_range(this.dimensions.width, this.dimensions.height, 32);
    cut = data.subarray(mm[0], mm[1]);
    Array.prototype.sort.call(cut, Util.numericSort);
  ...
  Caman.Filter.register('rgbGlitch', function(dir) {
    ...
    return this.processPlugin('rgbGlitch', [dir]);
  });

  Caman.Plugin.register('rgbGlitch', function(dir) {
